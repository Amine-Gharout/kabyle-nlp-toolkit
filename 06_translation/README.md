...
Tamazight translation using Meta's NLLB-200 (No Language Left Behind) with CTranslate2 for efficient inference.
...
| File | Description |
|------|-------------|
| `notebooks/translate_src_to_pivot.ipynb` | Translates English→Tamazight and French→Tamazight via NLLB |
| `notebooks/inference/nllb_ctranslate2_inference.ipynb` | Low-VRAM inference with CTranslate2 quantization |

## Method

Uses English/French as pivot languages → NLLB-200-3.3B → Tamazight. CTranslate2 provides int8/float16 quantization for running on limited GPU memory (3.5–6.5 GB).
...
# Translate: English → Tamazight (kab_Latn)
