...
Transcribing Tamazight speech from audio files and live radio streams.
...
| File | Description |
|------|-------------|
| `scripts/transcribe_tamazight_gemini.py` | Batch transcription of Tamazight audio via Gemini 2.5 Pro (uses `shared/gemini_utils.py`) |
| `notebooks/live_radio_transcription.ipynb` | Real-time transcription of Radio Chaine 2 with API key rotation |

## Method

Uses Google Gemini 2.5 Pro with a specialized prompt that encodes Tamazight linguistic rules (diacritics, y vs ɣ distinction) for accurate transcription.
